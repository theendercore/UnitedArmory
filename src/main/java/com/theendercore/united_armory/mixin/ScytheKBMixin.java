...
@Mixin(Player.class)
public abstract class ScytheKBMixin extends LivingEntity {
    protected ScytheKBMixin(EntityType<? extends LivingEntity> entityType, Level level) {
    ...
    float scytheCustomKB(float original, Entity entity, @Local(ordinal = 2) boolean crit) {
        var stack = getWeaponItem();
        if (crit && stack.getItem() instanceof UnnamedScythe) {
